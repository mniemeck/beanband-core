...
public class BandArranger extends Arranger {
	...
	public void annotate(Song song) throws InvalidMidiDataException {
		...
		if (!song.getElements().isEmpty())  {
			...
			if (!(songElement instanceof StyleChange)) {
				songElement.getOrCreateAnnotation(WarningAnnotation.class).addMessage("Song does not start with a style specification. Defaults assumed.");
			}
		}
		for (SongElement songElement : song.getElements()) {
			if (songElement instanceof StyleChange) {
				...
				if (styleChange.getTempo() > 0) {
					...
					songElement.getOrCreateAnnotation(WarningAnnotation.class).addMessage("Tempo must be greater than zero. Tempo not changed.");
				}
				if (bandMap.containsKey(currentStyle)) {
					currentBand = bandMap.get(currentStyle);
				} else {
					songElement.getOrCreateAnnotation(WarningAnnotation.class).addMessage("No matching band for style " + currentStyle + " found. Band not changed.");
				}
	...
	private void validateChordChanges(Bar bar, Band band) {
		...
		bar.getOrCreateAnnotation(WarningAnnotation.class).addMessage("Unsupported number of changes for Band " + band.getStyleName() + ". Excess chords ignored.");
	}
